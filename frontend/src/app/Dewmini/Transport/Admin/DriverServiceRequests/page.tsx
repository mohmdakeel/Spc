...
import { ArrowLeft, ChevronLeft, ChevronRight, Trash2, Edit, Printer, History as HistoryIcon } from 'lucide-react';
...
export default function DriverServiceRequestsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return requests.filter(r =>
      [
        r.id, r.vehicleNumber, r.driverName, r.epf, r.requestDate,
        r.lastServiceReadingKm, r.nextServiceReadingKm, r.currentReadingKm,
        r.adviceByVehicleOfficer, r.adviceByMechanic, r.hrApproval,
        ...(r.servicesNeeded ?? []),
      ...
      <div className="bg-white rounded-xl shadow-md p-3 sm:p-4">
                  ...
                  <tr key={r.id} className="hover:bg-orange-50 transition-colors">
                    ...
                    <td className="border px-2 py-1 text-xs">
                      <span className={`px-2 py-0.5 rounded text-[11px] ${
                        r.hrApproval === 'APPROVED' ? 'bg-green-100 text-green-700' :
                        r.hrApproval === 'REJECTED' ? 'bg-red-100 text-red-700' :
                    ...
                    <td className="border px-2 py-1 text-xs text-center">
                      <button onClick={() => toast.info('Hook to history')} className="px-1 text-orange-600"><HistoryIcon size={14} /></button>
                      <button onClick={() => toast.info('Hook to edit')} className="px-1 text-green-600"><Edit size={14} /></button>
                      <button onClick={() => setDeleteTarget(r)} className="px-1 text-red-600"><Trash2 size={14} /></button>
                    </td>
                  </tr>
                )) : (
                  <tr>
        ...
        <div className="flex justify-center items-center gap-2 mt-3">
          <button
            onClick={() => goTo(page - 1)}
            disabled={page === 1}
            className={`px-2 py-1 rounded ${page === 1 ? 'text-gray-400 cursor-not-allowed' : 'bg-orange-100 text-orange-600 hover:bg-orange-200'}`}
          >
            <ChevronLeft size={16} />
          </button>
          ...
          <button
            ...
            className={`px-2 py-1 rounded ${page === totalPages ? 'text-gray-400 cursor-not-allowed' : 'bg-orange-100 text-orange-600 hover:bg-orange-200'}`}
          >
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white p-4 rounded shadow-md w-80">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setDeleteTarget(null)} className="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">Cancel</button>
              <button onClick={onDelete} className="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700">Delete</button>
            </div>
          </div>
        </div>
      )}
