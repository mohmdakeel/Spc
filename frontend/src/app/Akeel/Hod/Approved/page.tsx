...
const chip = (s: string) => (
  <span className="inline-block text-[10px] px-1.5 py-[2px] rounded bg-blue-100 text-blue-800 leading-none">
    {s}
  </span>
);
...
export default function HODApprovedPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const list = await listByStatus('PENDING_MANAGEMENT'); // “Sent to Management”
      setRows(list || []);
    } finally {
      setLoading(false);
    }
...
<html>
<head>
<meta charset="utf-8"/><title>Sent to Management — Print</title>
    ...
    <div className="flex min-h-screen bg-orange-50">
      ...
      <main className="p-3 md:p-4 flex-1">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-[14px] md:text-lg font-bold text-orange-900">Sent to Management</h1>
        ...
        <div className="bg-white rounded-lg border border-orange-200 overflow-auto">
          {/* Column order mirrors My Requests / Pending views */}
          <table className="w-full table-fixed text-[10px] leading-[1.15]">
            ...
            <tbody className="divide-y">
              {loading && <tr><Td colSpan={9} className="px-2 py-6 text-center text-gray-500">Loading…</Td></tr>}
        ...
        <div className="p-4 text-[10.5px] leading-tight space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <section>
              ...
              <div>
                <b>Driver:</b> {request.assignedDriverName || '-'}{' '}
                {request.assignedDriverPhone ? `(${request.assignedDriverPhone})` : ''}
              </div>
            </section>
            ...
            <section className="md:col-span-2">
              ...
              <div>
                <b>Exit:</b> {fmtDT(request.gateExitAt)} •{' '}
                <span className="text-[9px] text-gray-600">O {request.exitOdometer ?? '-'}</span>
              </div>
              <div>
                <b>Entry:</b> {fmtDT(request.gateEntryAt)} •{' '}
                <span className="text-[9px] text-gray-600">O {request.entryOdometer ?? '-'}</span>
              </div>
            </section>
          </div>
        </div>
