...
import { Save, User, Mail, Key, Camera, Eye, EyeOff, Loader2 } from 'lucide-react';
  ...
  return (
    <>
      <header className="space-y-1">
        ...
        <h1 className="text-2xl font-bold text-orange-900">Management</h1>
        <p className="text-sm text-gray-600">
          Manage your account details, preferences, and credentials here.
        </p>
      </header>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="bg-white rounded-2xl border border-orange-200 shadow-sm p-6 space-y-3">
          ...
          <dl className="space-y-2 text-sm text-gray-700">
            ...
            <Row label="EPF Number" value={user.epfNo} />
          </dl>
        </section>
        ...
        <section className="bg-white rounded-2xl border border-orange-200 shadow-sm p-6 space-y-3">
          ...
          <p className="text-xs text-gray-500">
            Need to adjust access or contact details? Raise a request via Auth Service settings.
          </p>
        </section>
      </div>
      ...
      <section className="bg-white rounded-2xl border border-orange-200 shadow-sm p-6 space-y-4 max-w-2xl">
        ...
        <div className="flex items-center gap-4">
          <div className="w-20 h-20 rounded-full border-4 border-orange-200 overflow-hidden bg-orange-50 flex items-center justify-center">
            {user.imageUrl ? (
              <img src={user.imageUrl} alt={user.fullName || user.username || 'Management'} className="w-full h-full object-cover" />
            ) : (
    ...
    <label className="space-y-1 text-sm text-gray-700">
      ...
      <div className="relative">
        <input
          ...
          className="w-full border border-orange-200 rounded-lg px-3 py-2 pr-10 focus:outline-none focus:ring-2 focus:ring-orange-500"
          disabled={disabled}
        />
        <button
          type="button"
