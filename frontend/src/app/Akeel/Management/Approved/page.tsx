...
/* ---------------- helpers (match HOD Pending style) ---------------- */
...
/** Strip “Travelling Officer:” lines + phone tokens from description */
...
/** Officer from explicit fields or parsable text */
...
/* ---------- column widths (sum 100%): same balance as HOD Pending ---------- */
const COLS = [
  '12%', // RQ ID / Applied
  '17%', // Applicant / Dept
  '8%',  // Status
...
<tr>
  ...
  <td class="center">${r.status || 'APPROVED'}</td>
...
<html><head><meta charset="utf-8"/><title>Approved — Print</title><meta name="viewport" content="width=device-width, initial-scale=1"/>
<style>
...
.rq{font-weight:600;color:#166534}
...
.block{margin:8px 0 2px;font-weight:600;color:#166534}.sub{color:#666;font-size:11px}
...
<body>
  ...
  <div class="grid">
    ...
    <div><div class="block">Status</div>${r.status || 'APPROVED'}</div>
    ...
    <div className="space-y-4 p-3 md:p-4">
      <main>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 className="text-[14px] md:text-lg font-bold text-orange-900">Approved Requests</h1>
            <p className="text-xs text-gray-500">Latest requests that cleared the management review.</p>
          </div>
              ...
              {!loading && filtered.map((r: any) => {
                ...
                return (
                  ...
                  >
                      ...
                      {/* Status */}
                      <Td className="px-2 py-1 text-center">{chip(r.status || 'APPROVED')}</Td>
      ...
      {/* Row-click Details modal (basic only) */}
      {view
