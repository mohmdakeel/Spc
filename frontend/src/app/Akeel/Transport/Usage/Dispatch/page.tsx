...
export default function DispatchPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : 'Failed to load scheduled for dispatch');
      setRows([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-orange-50 p-4">
      <div className="bg-white rounded-xl border border-orange-200 p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-xl font-bold text-gray-800">Dispatch (Hand-over to Gate)</h1>
            <p className="text-sm text-orange-700">Trips scheduled and ready for Gate to mark EXIT.</p>
          </div>
            ...
            <tbody className="divide-y">
              ...
              {!filtered.length && (
                <tr><Td colSpan={6} className="text-center text-gray-500 py-6">
                  {loading ? 'Loading…' : 'Nothing pending dispatch'}
                </Td></tr>
              )}
            </tbody>
          </table>
