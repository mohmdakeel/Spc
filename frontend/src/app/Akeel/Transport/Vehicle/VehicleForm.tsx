...
// Convert string | Date | null to yyyy-MM-dd for <input type="date">
...
export default function VehicleForm({ vehicle, onSubmit, onClose }: Props) {
  // keep manufactureDate as yyyy-MM-dd string or null in form state
  ...
  useEffect(() => {
    if (vehicle) {
      setForm({
        ...vehicle,
        manufactureDate: toDateInputValue(vehicle.manufactureDate ?? null),
      });
    } else {
      setForm({ status: "ACTIVE", manufactureDate: null });
    }
  }, [vehicle]);
  ...
  const change = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (type === "date")   newVal = value || null;
      ...
      <form onSubmit={submit} className="bg-white rounded-xl shadow p-6 w-[420px]">
        ...
        <div className="space-y-2">
          ...
          <input className="border rounded px-3 py-2 w-full"
                 ...
                 value={toDateInputValue(form.manufactureDate ?? null)}
                 onChange={change} />
