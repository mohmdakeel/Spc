...
export default function VehicleHistoryModal({
  history, onClose,
}: { history: VehicleHistory[]; onClose: () => void }) {
  return (
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {history.map((h) => {
              let pretty = "-";
              if (h.previousData) {
                try { pretty = JSON.stringify(JSON.parse(h.previousData), null, 2); }
                catch { pretty = h.previousData; }
              }
              return (
                <tr key={h.id}>
                  ...
                  <td className="p-2"><pre className="max-w-[360px] whitespace-pre-wrap break-words">{pretty}</pre></td>
                </tr>
              );
            })}
          </tbody>
