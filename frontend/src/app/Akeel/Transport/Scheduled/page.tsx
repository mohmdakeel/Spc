...
export default function TransportScheduledPage() {
  ...
  const load = async () => {
    try {
      ...
      toast.error(e?.message || 'Failed to load scheduled vehicles');
      setScheduled([]);
      setDispatched([]);
    }
  };
    ...
    <div className="space-y-6 min-h-screen bg-orange-50 p-4">
      <div>
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-lg font-bold text-orange-900">Scheduled Vehicles</h1>
          <button
            className="px-3 py-2 rounded bg-orange-600 text-white text-sm font-semibold"
            onClick={load}
          >
            Refresh
          </button>
